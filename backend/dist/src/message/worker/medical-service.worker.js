...
exports.startServiceProvidedSubscriber = startServiceProvidedSubscriber;
 ...
 */
async function startServiceProvidedSubscriber() {
    ...
    await channel.bindQueue(DEAD_LETTER_QUEUE, DEAD_LETTER_EXCHANGE, SERVICE_PROVIDED_QUEUE);
    ...
    await channel.bindQueue(SERVICE_PROVIDED_QUEUE, MEDICAL_EXCHANGE, medical_service_model_1.ServiceRoutingKey.PROVIDED);
    // 4. Limit unacknowledged messages per consumer
    await channel.prefetch(1);
    console.log(`[*] Subscribed to queue '${SERVICE_PROVIDED_QUEUE}'`);
    // 5. Start consuming
    await channel.consume(SERVICE_PROVIDED_QUEUE, async (msg) => {
        if (!msg)
            return;
        try {
            const rawContent = JSON.parse(msg.content.toString());
            // Validate event structure against Zod schema (guarantees providedById)
            const event = medical_service_model_1.serviceProvidedEventSchema.parse(rawContent);
            // Process downstream business logic
            await handleServiceProvided(event);
            channel.ack(msg);
        }
        catch (error) {
            console.error("[RabbitMQ] Error processing service.provided event:", error);
 ...
 */
async function handleServiceProvided(event) {
    const { providedServiceId, visitId, serviceId, providedById, unitPrice, timestamp, } = event;
    console.log(`[Service Provided Event] Service ${serviceId} recorded for visit ${visitId} by staff member ${providedById}. Total: $${unitPrice}`);
    // Downstream tasks:
