...
const MAX_RETRIES = 3;
async function startPrescriptionWorker() {
    try {
        ...
        channel.consume(QUEUE_NAME, async (msg) => {
            ...
            try {
                ...
                console.log(`📥 [Worker] Processing "${event}" (Attempt ${currentRetryCount + 1})`);
                ...
                if (currentRetryCount < MAX_RETRIES) {
                    console.warn(`🔄 [Worker] Requeuing message for retry (${currentRetryCount + 1}/${MAX_RETRIES})...`);
                    // Requeue back into main queue for another attempt
                    channel.nack(msg, false, true);
                }
                else {
                    console.error(`☠️ [Worker] Max retries (${MAX_RETRIES}) reached. Routing message to DLQ!`);
...
async function handlePrescriptionEvent(event, data) {
