...
exports.startLabWorker = startLabWorker;
    ...
    try {
        ...
        channel.consume(MAIN_QUEUE, async (msg) => {
            ...
            try {
                ...
                console.log(`📥 [Lab Worker] Event "${routingKey}" (Attempt ${currentRetryCount + 1})`);
                switch (routingKey) {
                    case "lab.completed":
                    ...
                    case "lab.verified":
                        ...
                        break;
                    case "lab.ordered":
                        console.log(`📋 [Lab Worker] Lab ordered for visit ${payload.visitId}`);
                        break;
                    default:
                        console.warn(`⚠️ [Lab Worker] Unhandled routing key: ${routingKey}`);
                }
                // Acknowledge on success
                channel.ack(msg);
            }
            catch (error) {
                console.error(`❌ [Lab Worker] Failed to process ${routingKey}: ${error.message}`);
                if (currentRetryCount < MAX_RETRIES) {
                    console.warn(`🔄 [Lab Worker] Requeuing event for retry (${currentRetryCount + 1}/${MAX_RETRIES})...`);
                    channel.nack(msg, false, true); // Requeue = true
                }
                else {
                    console.error(`☠️ [Lab Worker] Max retries (${MAX_RETRIES}) reached. Directing to DLQ "${DLQ_NAME}"`);
...
async function handleLabCompleted(data) {
    console.log(`📱 [Notification] Lab result #${data.labResultId} is ready! Alerting attending doctor...`);
...
async function handleLabVerified(data) {
    console.log(`🔒 [Audit/EHR] Final verification recorded for lab #${data.labResultId} by user #${data.verifiedById}`);
