...
exports.startVitalSignsSubscribers = startVitalSignsSubscribers;
const rabbitmq_1 = require("../../config/rabbitmq");
const vital_sign_publisher_1 = require("../publisher/vital-sign.publisher");
...
async function startVitalSignsSubscribers() {
    ...
    await channel.bindQueue("critical_vitals_queue", MAIN_EXCHANGE, vital_sign_publisher_1.VitalSignsRoutingKey.CRITICAL);
    await channel.assertQueue("vital_signs_queue", queueOptions);
    await channel.bindQueue("vital_signs_queue", MAIN_EXCHANGE, vital_sign_publisher_1.VitalSignsRoutingKey.RECORDED);
    await channel.bindQueue("vital_signs_queue", MAIN_EXCHANGE, vital_sign_publisher_1.VitalSignsRoutingKey.UPDATED);
    await channel.prefetch(10);
    // 4. Consumer with Exponential Backoff Retry Strategy
    await channel.consume("critical_vitals_queue", async (msg) => {
        if (!msg)
            return;
        try {
 ...
 */
async function handleRetryWithBackoff(channel, msg, originalQueue) {
    ...
    if (currentRetryCount < MAX_RETRIES) {
        ...
        console.warn(`⚠️ [Retry ${nextRetryCount}/${MAX_RETRIES}] Scheduling retry for message in ${delayMs}ms...`);
        ...
        console.error(`❌ [Max Retries Reached] Exceeded ${MAX_RETRIES} attempts. Rejecting to DLQ...`);
        // nack with requeue=false automatically sends message to the configured DLX
        channel.nack(msg, false, false);
    }
}
async function processCriticalVitalsAlert(event) {
