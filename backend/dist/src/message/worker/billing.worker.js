...
exports.startBillingWorker = startBillingWorker;
...
async function setupBillingTopology() {
    ...
    await channel.bindQueue(MAIN_QUEUE, BILLING_EXCHANGE, "billing.invoice.generated");
    await channel.bindQueue(MAIN_QUEUE, BILLING_EXCHANGE, "billing.payment.received");
    await channel.bindQueue(MAIN_QUEUE, BILLING_EXCHANGE, "billing.invoice.settled");
    return channel;
}
async function startBillingWorker() {
    try {
        ...
        channel.consume(MAIN_QUEUE, async (msg) => {
            ...
            const currentRetryCount = Array.isArray(xDeath) && xDeath.length > 0 ? xDeath[0].count : 0;
            const routingKey = msg.fields.routingKey;
            try {
                const payload = JSON.parse(msg.content.toString());
                console.log(`📥 [Billing Worker] Processing "${routingKey}" (Attempt ${currentRetryCount + 1})`);
                switch (routingKey) {
                    ...
                    case "billing.invoice.settled":
                        ...
                        break;
                    default:
                        console.warn(`⚠️ [Billing Worker] Unhandled routing key: ${routingKey}`);
                }
                channel.ack(msg);
            }
            catch (error) {
                console.error(`❌ [Billing Worker] Error processing ${routingKey}: ${error.message}`);
                if (currentRetryCount < MAX_RETRIES) {
                    console.warn(`🔄 [Billing Worker] Retrying event (${currentRetryCount + 1}/${MAX_RETRIES})...`);
                    channel.nack(msg, false, true); // Requeue
                }
                else {
                    console.error(`☠️ [Billing Worker] Max retries reached. Routing to DLQ "${DLQ_NAME}"`);
...
async function handleInvoiceGenerated(data) {
    console.log(`🧾 [Billing Event] Invoice #${data.invoiceNumber} created for Visit #${data.visitId}. Total: TZS ${data.grandTotal}`);
...
async function handlePaymentReceived(data) {
    console.log(`💳 [Billing Event] Payment of TZS ${data.amount} received via ${data.paymentMethod}. Balance remaining: TZS ${data.balanceRemaining}`);
...
async function handleInvoiceSettled(data) {
    console.log(`🎉 [Billing Event] Invoice #${data.invoiceId} FULLY SETTLED. Unlocking patient discharge workflow...`);
