...
const vital_sign_publisher_1 = require("../../message/publisher/vital-sign.publisher");
...
class VitalSignsService {
    ...
    async create(recordedById, input) {
        ...
        const [visit, existingVitals] = await Promise.all([
            ...
            database_1.prisma.vitalSigns.findUnique({
        ...
        if (visit.status !== client_1.VisitStatus.IN_PROGRESS) {
            throw new custom_error_1.BadRequestError(`CANNOT_RECORD_VITALS: Visit is not active (Current status: ${visit.status}). Vital signs can only be recorded for visits with status IN_PROGRESS.`);
        }
        if (existingVitals) {
            throw new custom_error_1.ConflictError(`VITAL_SIGNS_ALREADY_EXISTS: ${input.visitId}`);
        }
        ...
        const vitals = await database_1.prisma.$transaction(async (tx) => {
            const createdVitals = await tx.vitalSigns.create({
                data: {
                    visitId: input.visitId,
                    temperature: input.temperature !== undefined
                        ? new client_1.Prisma.Decimal(input.temperature)
                        : undefined,
                    ...
                    spo2: input.spo2,
                    weight: new client_1.Prisma.Decimal(input.weight),
                    height: input.height !== undefined
                        ? new client_1.Prisma.Decimal(input.height)
                        : undefined,
                    bmi: new client_1.Prisma.Decimal(computedBmi),
        ...
        await Promise.all([
            (0, vital_sign_publisher_1.publishVitalSignsRecordedEvent)(eventPayload),
            (0, vital_sign_publisher_1.publishRealtimeAlertEvent)(eventPayload),
        ]);
        ...
        if (redis_1.redisClient.status === "ready" || redis_1.redisClient.status === "connect") {
            await Promise.all([
                redis_1.redisClient.setex(`vitals:visit:${input.visitId}`, VITAL_SIGNS_CACHE_TTL, JSON.stringify(vitals)),
                redis_1.redisClient.del(`visit:${input.visitId}`),
                redis_1.redisClient.del(`patient:${visit.patientId}:visits`),
            ]);
        }
        return vitals;
    }
    ...
    async findByVisitId(visitId) {
        ...
        if (redis_1.redisClient.status === "ready" || redis_1.redisClient.status === "connect") {
            const cachedVitals = await redis_1.redisClient.get(cacheKey);
            if (cachedVitals) {
                return JSON.parse(cachedVitals);
            }
        }
        const vitals = await database_1.prisma.vitalSigns.findUnique({
            where: { visitId },
            include: {
                recordedBy: {
        ...
        if (!vitals) {
            throw new custom_error_1.NotFoundError(`VITAL_SIGNS_NOT_FOUND: ${visitId}`);
        }
        if (redis_1.redisClient.status === "ready" || redis_1.redisClient.status === "connect") {
            await redis_1.redisClient.setex(cacheKey, VITAL_SIGNS_CACHE_TTL, JSON.stringify(vitals));
        }
        return vitals;
    }
    /**
    ...
    async update(id, input) {
        const currentVitals = await database_1.prisma.vitalSigns.findUnique({
            where: { id },
            include: { visit: { select: { patientId: true, status: true } } },
        });
        if (!currentVitals) {
            throw new custom_error_1.NotFoundError(`VITAL_SIGNS_RECORD_NOT_FOUND: ${id}`);
        }
        if (currentVitals.visit.status !== client_1.VisitStatus.IN_PROGRESS) {
            throw new custom_error_1.BadRequestError(`CANNOT_UPDATE_VITALS: Associated visit is no longer active (Status: ${currentVitals.visit.status}).`);
        }
        ...
        const updatedVitals = await database_1.prisma.$transaction(async (tx) => {
            const vitals = await tx.vitalSigns.update({
                where: { id },
                data: {
                    ...(input.temperature !== undefined && {
                        temperature: new client_1.Prisma.Decimal(input.temperature),
                    }),
                    ...
                    ...(input.height !== undefined && {
                        height: new client_1.Prisma.Decimal(input.height),
                    }),
                    bmi: new client_1.Prisma.Decimal(computedBmi),
        ...
        await (0, vital_sign_publisher_1.publishVitalSignsUpdatedEvent)({
        ...
        if (redis_1.redisClient.status === "ready" || redis_1.redisClient.status === "connect") {
            await Promise.all([
                redis_1.redisClient.del(`vitals:visit:${updatedVitals.visitId}`),
                redis_1.redisClient.del(`visit:${updatedVitals.visitId}`),
                redis_1.redisClient.del(`patient:${currentVitals.visit.patientId}:visits`),
            ]);
        }
    ...
    async findMany(query) {
        const { page = 1, limit = 20, patientId, priority, startDate, endDate, } = query;
        ...
        const [total, records] = await Promise.all([
            database_1.prisma.vitalSigns.count({ where }),
            database_1.prisma.vitalSigns.findMany({
                where,
                skip,
                take: limit,
